...
| 场景 | JQL 策略 |
|---|---|
| 写日报 / 今天做了什么 | `assignee = currentUser() AND updated >= startOfDay() ORDER BY updated DESC` |
| 写周报 / 这周进度 | `assignee = currentUser() AND updated >= startOfWeek() ORDER BY updated DESC` |
| 我的待办 / 当前任务 | `assignee = currentUser() AND resolution = Unresolved AND updated >= -30d ORDER BY updated DESC` |
| 查全部（用户明确说"所有"/"包括旧的"）| 不加时间限制 |

**日报/周报场景说明**：
- 重点是"今天/本周有发生变化"的工单，不限制工单创建时间，也不过滤 resolution
- `updated` 涵盖状态变更、工时录入、评论、字段修改等所有操作
- 已完成的工单如果今天有变动，同样应纳入日报
